...
case class Arguments(
    ...
    maxAncestorLevelsToCheckForFiles: Int = Arguments.DefaultMaxAncestorLevelsToCheckForFiles
)
...
object Arguments {
  ...
  private def nextState(
    ...
    arg match {
      case ExtensionsArgument if remainingArgs.nonEmpty =>
        ...
        Success(
          state.copy(
            argsToParse = remainingArgs.tail,
        ...
        Try(remainingArgs.head.toInt) match {
          case Success(maxLevel) =>
            Success(
              state.copy(
                argsToParse = remainingArgs.tail,
                arguments = state.arguments.copy(maxAncestorLevelsToCheckForFiles = maxLevel)
              )
            )
          case Failure(_) =>
            Failure(
  ...
  private val argumentDescriptionsString: String = {
    ...
    argumentDescriptions map { case (argument, description) =>
      argument + " " * (longestArgumentLength - argument.length + 1) + description
    } mkString "\n"
  }

  val usage: String =
    "Usage: ListUntaggedAlbums [options] directory\n\nOptions:\n\n" + argumentDescriptionsString
}
